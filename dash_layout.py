...
app.layout = html.Div(children=[  # TASK 3A
    html.H1('Car Automobile Components',
            style={'textAlign': 'center', 'color': '#503D36',
                   'font-size': 24}),    # outer division starts
    html.Div([
...
def display_selected_drive_charts(value):
    ...
    df = filtered_df.groupby(['drive-wheels','body-style'], as_index=False).mean()

    fig1 = px.pie(df, values = "price", names = "body-style", title="Pie Chart")
    fig2 = px.bar(df, x = "body-style", y = "price", title = "Bar Chart")
